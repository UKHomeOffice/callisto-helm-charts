...
# Test Connection
if
  openssl s_client -connect $BOOTSTRAP_SERVER1 -key $service_alias-key.pem -cert $service_alias-certificate.pem -brief
then
...
if
  keytool -importkeystore -srckeystore $service_alias-key-pair.p12 -srcstorepass $password -destkeystore keystore.jks -srcstoretype pkcs12 -alias shared -storepass $password -keypass $password
then
